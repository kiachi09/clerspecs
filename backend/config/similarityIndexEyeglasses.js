...
const getEyeglasses = asyncHandler(async () => {
  const data = await Product.find({ category: "Eyeglasses" });
...
const saveSimilarityAll = asyncHandler(async (similarityResults) => {
  try {
    ...
    return {
      status: 200,
      message: "Similarity indexes For EyeGlasses updated successfully!",
    };
  } catch (error) {
    return {
      status: 500,
...
const similarityIndexEyeglasses = async () => {
  try {
    // getting all the products from the database
    const queryData = await getEyeglasses();
    ...
    console.log(
      "here are the number of embeddings for eye",
...
export default similarityIndexEyeglasses;
