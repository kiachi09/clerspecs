...
const getSunglasses = asyncHandler(async () => {
...
const saveSimilarityAll = asyncHandler(async (similarityResults) => {
  try {
    ...
    return {
      status: 200,
      message: "Similarity indexes For SunGlasses updated successfully!",
    };
  } catch (error) {
    return {
      status: 500,
      error:
        "Error updating similarity indexes for Sunglasses: " + error.message,
    };
  }
});

const similarityIndexSunglasses = async () => {
  try {
    // getting all the products from the database
    const queryData = await getSunglasses();
    ...
    console.log(
      "here are the number of embeddings for sun",
    ...
    return {
      status: 500,
      error:
        "Error updating similarity indexes for Eyeglasses: " + error.message,
    };
  }
};

export default similarityIndexSunglasses;
